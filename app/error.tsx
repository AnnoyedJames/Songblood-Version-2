...
export default function Error({
  ...
  useEffect(() => {
    if (isRedirectError) {
      setRedirecting(true)
      // Use window.location to force a client-side navigation
      window.location.href = "/login?reason=error-redirect"
    }
  }, [isRedirectError])
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md w-full text-center">
          ...
          <p className="mb-4">Please wait while we redirect you to the login page.</p>
        </div>
      </div>
    )
  }
